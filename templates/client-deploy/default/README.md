...
- `secrets/` — encrypted secret material placeholders, with `core-01.yaml.example` as the canonical example shape
...
- replace `secrets/core-01.yaml.example` with real encrypted material;
...
- `rauthy.env` materialized from the encrypted `core-01.yaml` secret via the optional `sops-nix` baseline shipped by `void-core`

...
| `tool-gateway` | `foundation primitive` | provides the shared assistant tool-call policy and preview/apply confirmation boundary | foundation exposes dependency-free contracts for `ToolGateway` decisions, audit decision shape, and `assistant_confirm_tokens` storage semantics so chat, external MCP, and autonomous runner surfaces can share one security boundary | concrete product tools, graph mutations, deployment policy, and database client wiring remain downstream-owned |

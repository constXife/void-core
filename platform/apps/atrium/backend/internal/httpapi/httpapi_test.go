...
func TestCalendarEventsProxyConfiguredRequest(t *testing.T) {
	manager, err := auth.NewManager(context.Background(), nil, auth.Config{
		LocalEnabled: true,
		CookieSecret: "test-secret",
	})
	...
	handler := Handler(Deps{
		...
		KnowledgeProxyHTTPClient: &http.Client{
			Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
				if r.URL.Path != "/api/knowledge/v1/calendar/events" {
					t.Fatalf("expected upstream path /api/knowledge/v1/calendar/events, got %q", r.URL.Path)
				}
				gotAuth = r.Header.Get("Authorization")
				gotSubject = r.Header.Get(knowledgeAuthSubjectHeader)
				gotEmail = r.Header.Get(knowledgeUserEmailHeader)
				...
				gotReminders = r.URL.Query().Get("include_reminders")
				return &http.Response{
					StatusCode: http.StatusOK,
					Header: http.Header{
						"Content-Type": []string{"application/json"},
					},
					Body: io.NopCloser(strings.NewReader(`{"summary":{"visible_count":1},"items":[]}`)),
				}, nil
			}),
		},
	})

	req := httptest.NewRequest(http.MethodGet, "/api/calendar/events?window=week&include_reminders=true", nil)
	req.AddCookie(&http.Cookie{Name: "atrium_session", Value: encoded})
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	...
	if gotSubject != "viewer-subject" {
		t.Fatalf("expected auth subject viewer-subject, got %q", gotSubject)
	}
	if gotEmail != "viewer@example.com" {
		t.Fatalf("expected forwarded email, got %q", gotEmail)
	}
	if gotRole != "member" {
		t.Fatalf("expected forwarded role, got %q", gotRole)
	...
	if gotWindow != "week" {
		t.Fatalf("expected window=week, got %q", gotWindow)
	}
	if gotReminders != "true" {
		t.Fatalf("expected include_reminders=true, got %q", gotReminders)
	}
}
